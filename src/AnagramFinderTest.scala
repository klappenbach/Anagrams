class AnagramFinderTest extends org.scalatest.FunSuite {
  test("simple two word anagram is found") {
    ...
    assert(anagrams.size == 1 && anagrams.exists(_.size == 2), "didn't find the anagrams: " + anagrams)
  }
  ...
  test("finds two groups with anagrams"){
    ...
    assert(anagrams.size == 2 && anagrams.forall(_.size == 2), "didn't find the anagrams: " + anagrams)
  }
